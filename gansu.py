...
# 从本地读取 GeoJSON 文件
with open('甘肃省.json', 'r', encoding='utf-8') as file:
...
ax.set_extent([85.4, 110.1, 30.8, 43.9], crs=ccrs.PlateCarree())
shp_file_path = 'shp/甘肃省.shp'
...
inset_border = Rectangle(
    (0.23, 0.6),  # left and bottom position of the inset (matching the inset position)
...
# ax.legend(handles=legend_elements, loc='lower right', fontsize=12, title='Legend', bbox_to_anchor=(0.4, 0.02))
# 从本地读取中国的 GeoJSON 文件
...
ax_china_inset = fig.add_axes([0.24, 0.6, 0.15, 0.3], projection=equal_area_crs)  # [left, bottom, width, height]
...
for feature in china_data['features']:
    ...
    if '甘肃省' in feature['properties']['name']:  # 假设属性中有 name 或类似的字段
